...
export default function ContentSection({ educationalContent, user, loadAdminData, showToast }: { educationalContent: any[]; user: any; loadAdminData: () => void; showToast: (msg: string, type?: any) => void }) {
  ...
  const handleContentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        loadAdminData(); showToast(editingContent ? t('adm.ct.courseUpdated') : t('adm.ct.courseAdded'), 'success');
      }
    } catch { showToast(t('adm.ct.errorRetry'), 'error'); }
  };
  ...
  const handleDeleteContent = async (contentId: number) => {
    try {
      ...
      if (r.ok) { loadAdminData(); showToast(t('adm.ct.courseDeleted'), 'success'); }
      else { const d = await r.json(); showToast(`Hitilafu: ${d.error || t('adm.ct.deleteFailed')}`, 'error'); }
    } catch { showToast(t('adm.ct.deleteError'), 'error'); }
  };
  ...
  const handleTogglePublish = async (contentId: number, currentStatus: boolean) => {
    try {
      ...
      if (r.ok) { loadAdminData(); showToast(currentStatus ? t('adm.ct.hidden') : t('adm.ct.published'), 'success'); }
    } catch { showToast(t('adm.ct.errorDot'), 'error'); }
  };
  ...
  const handleAiGenerateCourse = async (saveNow: boolean) => {
    if (!aiCoursePrompt.trim()) { showToast(t('adm.ct.enterTopic'), 'error'); return; }
    if (saveNow && !aiCoursePreview) { showToast(t('adm.ct.previewFirst'), 'error'); return; }
    ...
    } catch { setAiCourseError(t('adm.ct.networkErr')); }
  ...
  const handleGenerateLessonsWithAI = async (saveToCourse: boolean) => {
    ...
    if (!aiTopicPrompt.trim()) { showToast(t('adm.ct.enterDesc'), 'error'); return; }
    if (saveToCourse && (!aiGeneratedLessons || aiGeneratedLessons.length === 0)) { showToast(t('adm.ct.noLessonsToSave'), 'error'); return; }
    setAiError(null);
    ...
    try {
      ...
      if (saveToCourse) { setAiGeneratedLessons(null); setShowAiLessonGenerator(false); setAiTopicPrompt(''); await handleManageLessons(managingLessons); showToast(t('adm.ct.lessonsSaved'), 'success'); return; }
      setAiGeneratedLessons(Array.isArray(data?.lessons) ? data.lessons : []);
    } catch { setAiError(t('adm.ct.lessonsError')); }
  ...
  const handleLessonSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (r.ok) { setShowLessonForm(false); setEditingLesson(null); setLessonForm({ language: 'sw', title: '', content: '', duration_minutes: 15, lesson_type: 'text', video_url: '' }); handleManageLessons(managingLessons); showToast(editingLesson ? t('adm.ct.lessonUpdated') : t('adm.ct.lessonAdded'), 'success'); }
    } catch { showToast(t('adm.ct.lessonSaveError'), 'error'); }
  };

  const handleDeleteLesson = async (lessonId: number) => {
    try {
      const r = await fetch(`/api/admin/educational-content/${managingLessons.id}/lessons/${lessonId}`, { method: 'DELETE' });
      if (r.ok) { handleManageLessons(managingLessons); showToast(t('adm.ct.lessonDeleted'), 'success'); }
    } catch { showToast(t('adm.ct.lessonDeleteError'), 'error'); }
  };

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-base font-semibold text-foreground">{t('adm.ct.title')}</h2>
          ...
          <div className="w-full max-w-2xl rounded-2xl bg-overlay border border-border max-h-[90vh] overflow-y-auto shadow-xl">
            <div className="p-6">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-base font-semibold text-foreground">{editingContent ? t('adm.ct.edit') : t('adm.ct.new')}</h3>
              ...
              {(contentFormTab === 'ai' && !editingContent) && (
                <div className="space-y-4">
                  {!aiCoursePreview ? (
                    <>
                      <div><label className={dkLabel}>{t('adm.ct.aiDescribe')} *</label><textarea value={aiCoursePrompt} onChange={e => setAiCoursePrompt(e.target.value)} className={`${dkInput} resize-none`} rows={3} placeholder={t('adm.ct.aiDescribePh')} /></div>
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div><label className={dkLabel}>{t('adm.ct.level')}</label><select value={aiCourseDiff} onChange={e => setAiCourseDiff(e.target.value as any)} className={dkSelect}><option value="beginner">{t('adm.ct.lvBeginner')}</option><option value="intermediate">{t('adm.ct.lvIntermediate')}</option><option value="advanced">{t('adm.ct.lvAdvanced')}</option></select></div>
                        <div><label className={dkLabel}>{t('adm.ct.category')}</label><select value={aiCourseCategory} onChange={e => setAiCourseCategory(e.target.value)} className={dkSelect}><option value="Biashara">Biashara</option><option value="Fedha">Fedha</option><option value="Uongozi">Uongozi</option><option value="Akiba">Akiba</option><option value="Teknolojia">Teknolojia</option></select></div>
                        <div><label className={dkLabel}>{t('adm.ct.lessonCount')}</label><input type="number" min={1} max={20} value={aiCourseLessonCount} onChange={e => setAiCourseLessonCount(parseInt(e.target.value||'1'))} className={dkInput} /></div>
                      </div>
                      ...
                      <div className="flex gap-2 pt-1">
                        <button type="button" onClick={() => { setShowContentForm(false); setEditingContent(null); }} className="flex-1 py-2.5 rounded-xl border border-border text-muted-foreground text-sm hover:bg-foreground/5 transition-colors">{t('adm.ct.cancel')}</button>
                    ...
                    <>
                      <div className="rounded-xl bg-orange-500/5 border border-orange-500/20 p-4">
                        <p className="text-xs text-orange-500 font-semibold mb-3">{t('adm.ct.aiPreview')}</p>
                      ...
                      <div className="flex gap-2 pt-1">
                        ...
                        <button onClick={() => handleAiGenerateCourse(true)} disabled={aiCourseSaving} className="flex-1 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium disabled:opacity-40 transition-colors">{aiCourseSaving ? t('adm.ct.saving') : '✓ Hifadhi Kozi na Masomo Yote'}</button>
                      </div>
                    </>
                  )}
                </div>
              ...
              {(contentFormTab === 'manual' || editingContent) && (
                <form onSubmit={handleContentSubmit} className="space-y-3">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div><label className={dkLabel}>{t('adm.ct.title2')} *</label><input type="text" value={contentForm.title} onChange={e => setContentForm({...contentForm, title: e.target.value})} className={dkInput} required /></div>
                    <div><label className={dkLabel}>{t('adm.ct.category')} *</label><select value={contentForm.category} onChange={e => setContentForm({...contentForm, category: e.target.value})} className={dkSelect} required><option value="">{t('adm.ct.chooseCategory')}</option><option value="Biashara">Biashara</option><option value="Uongozi">Uongozi</option><option value="Fedha">Fedha</option><option value="Teknolojia">Teknolojia</option></select></div>
                  </div>
                  <div><label className={dkLabel}>{t('adm.ct.shortDesc')} *</label><textarea value={contentForm.description} onChange={e => setContentForm({...contentForm, description: e.target.value})} className={`${dkInput} resize-none`} rows={3} required /></div>
                  <div><label className={dkLabel}>{t('adm.ct.fullContent')} *</label><textarea value={contentForm.content} onChange={e => setContentForm({...contentForm, content: e.target.value})} className={`${dkInput} resize-none`} rows={6} required /></div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div><label className={dkLabel}>{t('adm.ct.duration')}</label><input type="text" value={contentForm.duration} onChange={e => setContentForm({...contentForm, duration: e.target.value})} className={dkInput} placeholder="2 masaa" /></div>
                    <div><label className={dkLabel}>{t('adm.ct.level')}</label><select value={contentForm.difficulty_level} onChange={e => setContentForm({...contentForm, difficulty_level: e.target.value})} className={dkSelect}><option value="beginner">{t('adm.ct.lvBeginner')}</option><option value="intermediate">{t('adm.ct.lvIntermediate')}</option><option value="advanced">{t('adm.ct.lvAdvanced')}</option></select></div>
                    <div><label className={dkLabel}>{t('adm.ct.imageUrl')}</label><input type="url" value={contentForm.image_url} onChange={e => setContentForm({...contentForm, image_url: e.target.value})} className={dkInput} /></div>
                  </div>
                  <div className="rounded-xl bg-foreground/[0.03] border border-border p-4 space-y-3">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t('adm.ct.courseOptions')}</p>
                    ...
                    <label className="flex items-center gap-2.5 cursor-pointer"><input type="checkbox" checked={contentForm.certificates_enabled} onChange={e => setContentForm({...contentForm, certificates_enabled: e.target.checked})} className="accent-orange-500 w-4 h-4" /><div><span className="text-sm text-foreground/60">{t('adm.ct.certOnComplete')}</span><p className="text-xs text-foreground/30 mt-0.5">{t('adm.ct.certNote')}</p></div></label>
                    {contentForm.certificates_enabled && (
                      <div className="pl-6"><label className={dkLabel}>{t('adm.ct.passThreshold')}</label><div className="flex items-center gap-3"><input type="range" min={50} max={100} step={5} value={contentForm.pass_threshold} onChange={e => setContentForm({...contentForm, pass_threshold: parseInt(e.target.value)})} className="flex-1 accent-orange-500" /><span className="text-sm font-semibold text-orange-500 w-12 text-right">{contentForm.pass_threshold}%</span></div></div>
                    )}
                  </div>
                  <div className="flex gap-2 pt-2">
                    <button type="button" onClick={() => { setShowContentForm(false); setEditingContent(null); }} className="flex-1 py-2.5 rounded-xl border border-border text-muted-foreground text-sm hover:bg-foreground/5 transition-colors">{t('adm.ct.cancel')}</button>
                    <button type="submit" className="flex-1 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white text-sm font-medium transition-colors">{editingContent ? t('adm.ct.saveChanges') : t('adm.ct.save')}</button>
                  </div>
                </form>
              )}
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="w-full max-w-3xl rounded-2xl bg-overlay border border-border max-h-[90vh] overflow-y-auto shadow-xl">
            <div className="p-6">
              ...
              <div className="mb-4"><p className="text-xs font-semibold text-muted-foreground mb-2">{t('adm.ct.desc')}</p><p className="text-sm text-foreground/60">{viewingContent.description}</p></div>
              <div className="mb-5"><p className="text-xs font-semibold text-muted-foreground mb-2">{t('adm.ct.content')}</p><div className="text-sm text-foreground/50 whitespace-pre-wrap leading-relaxed">{viewingContent.content}</div></div>
              <div className="flex items-center justify-between pt-4 border-t border-border">
                ...
                <div className="flex gap-2">
                  <button onClick={() => { setViewingContent(null); handleEditContent(viewingContent); }} className="px-3.5 py-2 rounded-xl bg-foreground/5 hover:bg-foreground/10 text-foreground/50 text-sm transition-colors">{t('adm.ct.edit2')}</button>
      ...
      {managingLessons && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="w-full max-w-5xl rounded-2xl bg-overlay border border-border max-h-[90vh] overflow-y-auto shadow-xl">
            <div className="p-6">
              <div className="flex items-center justify-between mb-5">
                <div><h3 className="text-base font-semibold text-foreground">{t('adm.ct.manageLessons')}</h3><p className="text-xs text-muted-foreground mt-0.5">{managingLessons.title}</p></div>
                  ...
                  <div className="flex items-start gap-4">
                    <div className="flex-1 space-y-3">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        ...
                        <div><label className={dkLabel}>{t('adm.ct.level')}</label><select value={aiDifficulty} onChange={e => setAiDifficulty(e.target.value as any)} className={dkSelect}><option value="beginner">{t('adm.ct.lvBeginner')}</option><option value="intermediate">{t('adm.ct.lvIntermediate')}</option><option value="advanced">{t('adm.ct.lvAdvanced')}</option></select></div>
                        <div><label className={dkLabel}>{t('adm.ct.lessonCount')}</label><input type="number" min={1} max={20} value={aiLessonCount} onChange={e => setAiLessonCount(parseInt(e.target.value||'1'))} className={dkInput} /></div>
                        <div className="flex items-end"><button onClick={() => handleGenerateLessonsWithAI(false)} disabled={aiGenerating||aiSaving} className="w-full py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white text-xs font-semibold disabled:opacity-40 transition-colors">{aiGenerating ? 'Inatengeneza...' : t('adm.ct.preview')}</button></div>
                      </div>
                      <div><label className={dkLabel}>{t('adm.ct.lessonTopic')}</label><textarea value={aiTopicPrompt} onChange={e => setAiTopicPrompt(e.target.value)} className={`${dkInput} resize-none`} rows={2} placeholder={t('adm.ct.lessonTopicPh')} /></div>
                      ...
                      {aiGeneratedLessons && (
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <button onClick={() => handleGenerateLessonsWithAI(true)} disabled={aiSaving||aiGenerating} className="px-3 py-1.5 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 text-xs font-semibold border border-emerald-500/20 disabled:opacity-40">{aiSaving ? t('adm.ct.saving') : t('adm.ct.saveToCourse')}</button>
                          </div>
              ...
              {showLessonForm && (
                <div className="rounded-xl bg-foreground/[0.03] border border-border p-4 mb-5">
                  <p className="text-sm font-semibold text-foreground mb-3">{editingLesson ? t('adm.ct.editLesson') : t('adm.ct.newLesson')}</p>
                  <form onSubmit={handleLessonSubmit} className="space-y-3">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div><label className={dkLabel}>{t('adm.ct.lessonTitle')} *</label><input type="text" value={lessonForm.title} onChange={e => setLessonForm({...lessonForm, title: e.target.value})} className={dkInput} required /></div>
                      <div><label className={dkLabel}>{t('adm.ct.lessonDuration')} *</label><input type="number" value={lessonForm.duration_minutes} onChange={e => setLessonForm({...lessonForm, duration_minutes: parseInt(e.target.value)})} className={dkInput} min="1" required /></div>
                    </div>
                    ...
                    <div><label className={dkLabel}>{t('adm.ct.lessonContent')} *</label><textarea value={lessonForm.content} onChange={e => setLessonForm({...lessonForm, content: e.target.value})} className={`${dkInput} resize-none`} rows={6} placeholder={t('adm.ct.lessonContentPh')} required /></div>
                    <div className="flex gap-2">
                      <button type="button" onClick={() => { setShowLessonForm(false); setEditingLesson(null); }} className="flex-1 py-2.5 rounded-xl border border-border text-muted-foreground text-sm hover:bg-foreground/5 transition-colors">{t('adm.ct.cancel')}</button>
                      <button type="submit" className="flex-1 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white text-sm font-medium transition-colors">{editingLesson ? t('adm.ct.saveChanges') : 'Hifadhi Somo'}</button>
                    </div>
                  </form>
                </div>
              )}
              ...
              <div className="space-y-3">
                ...
                {lessons.length > 0 ? lessons.map(lesson => (
                  ...
                  <div className="text-center py-10">
                    ...
                    <p className="text-sm text-muted-foreground">{t('adm.ct.noLessonsYet')}</p>
                    <p className="text-xs text-foreground/20 mt-1">{t('adm.ct.clickNewLesson')}</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <table className="min-w-full">
          ...
          <tbody className="divide-y divide-border">
            {educationalContent.length > 0 ? educationalContent.map(c => (
              ...
              <tr><td colSpan={5} className="px-4 py-12 text-center"><BookOpenIcon className="h-8 w-8 mx-auto text-foreground/10 mb-3" /><p className="text-sm text-muted-foreground">{t('adm.ct.noCoursesYet')}</p></td></tr>
            )}
          </tbody>
        </table>
      </div>
