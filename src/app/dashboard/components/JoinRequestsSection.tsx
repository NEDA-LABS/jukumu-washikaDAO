  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-1">
        <h2 className="text-base font-semibold text-foreground">Maombi ya Kujiunga</h2>
        {pending.length > 0 && (
      ...
      {joinRequests.length === 0 ? (
        <div className="rounded-xl bg-card border border-border p-12 text-center shadow-sm">
          ...
          <p className="text-sm text-muted-foreground">Hakuna maombi ya kujiunga kwa sasa</p>
        </div>
      ) : (
            ...
            <div key={req.id} className={`rounded-xl border p-4 bg-card shadow-sm ${req.status === 'pending' ? 'border-orange-500/30' : 'border-border'}`}>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">Kujiunga na: <span className="text-foreground/60 font-medium">{req.group_name}</span></p>
              ...
              {req.message && (
                <div className="mt-3 px-3 py-2 rounded-lg bg-foreground/[0.03] border border-border">
                  <p className="text-xs text-foreground/50"><span className="text-muted-foreground">Ujumbe:</span> {req.message}</p>
                </div>
              )}
