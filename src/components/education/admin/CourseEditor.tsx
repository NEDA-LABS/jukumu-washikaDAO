...
export default function CourseEditor({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      {/* Title */}
      <div>
        <label className="block text-sm text-muted-foreground mb-1.5">Jina la kozi</label>
        <input
          type="text"
          value={title}
      ...
      {/* Description */}
      <div>
        <label className="block text-sm text-muted-foreground mb-1.5">Maelezo</label>
        <textarea
          value={description}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm text-muted-foreground mb-1.5">Kiwango</label>
          <select
            value={difficulty}
            onChange={(e) => setDifficulty(e.target.value as typeof difficulty)}
            className="w-full bg-muted border border-border text-foreground rounded-lg px-3 py-2.5 text-sm"
          >
        ...
        <div>
          <label className="block text-sm text-muted-foreground mb-1.5">Lugha</label>
          <select
            value={language}
      ...
      {/* Category */}
      <div>
        <label className="block text-sm text-muted-foreground mb-1.5">Kundi</label>
        <select
          value={categoryId}
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className={`relative inline-flex h-6 w-11 shrink-0 rounded-full border-2 border-transparent transition-colors duration-200 ${
            isPublished ? 'bg-primary' : 'bg-border'
          }`}
        >
          <span
